...
void draw() {
  ...
  if (skeletonArray.size() > 0) {
    ...
    if (skeleton.isTracked()) {
      ...
      PVector leftFoot = joints[KinectPV2.JointType_FootLeft].getPosition().copy();
      PVector rightFoot = joints[KinectPV2.JointType_FootRight].getPosition().copy();
      float feetDist = leftFoot.dist(rightFoot);
      println(feetDist);
      if (feetDist > .4) {
        textSize(60);
        fill(0, 255, 0);
        text("Feet apart", 100, 100);
        println("FEET APART");
      } else if (feetDist < .25) {
        textSize(60);
        fill(255, 0, 0);
        text("Feet together", 100, 100);
        println("FEET TOGETHER");
      }
    }
  }
